...
#define MESH_TEST_ADDR_MAX_IDX     2  //当前测试中，有多少个n+1节点
...
static uint8_t           TX_DATA[10] = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h',
                              'i', 'j'};
...
static void lwns_mesh_recv(lwns_controller_ptr ptr,
                           const lwns_addr_t *sender, uint8_t hops)
{
    uint8_t *data = lwns_buffer_dataptr();
    uint8_t Len = lwns_buffer_datalen();
...
//           (char *)lwns_buffer_dataptr(), lwns_buffer_datalen());
    PRINTF("mesh %d received from %02x.%02x.%02x.%02x.%02x.%02x len=%d\r\n",
           get_lwns_object_port(ptr),
           sender->v8[0], sender->v8[1],
           sender->v8[2], sender->v8[3],
           sender->v8[4], sender->v8[5],
           lwns_buffer_datalen());
    ...
    PRINTF("\r\n");
    if(sender->v8[4] == 0x01)//接收到对应组的数据
    {
        if(sender->v8[5] == 0x01)//节点1：环境监测
        {
            Ble_Node_1.HC_SR501_status = data[0];
            Ble_Node_1.temperature = ((data[2] << 8) | data[1]) / 10.0f;
            Ble_Node_1.humidity = ((data[4] << 8) | data[3]) / 10.0f;
            Ble_Node_1.Lux = (data[6] << 8) | data[5];
            PRINTF("Node1 => PIR:%d Temp:%d Humi:%d Lux:%d\r\n",
                   Ble_Node_1.HC_SR501_status,
                   (uint8_t)(Ble_Node_1.temperature),
                   (uint8_t)Ble_Node_1.humidity,
                   Ble_Node_1.Lux);
        }
        else if(sender->v8[5] == 0x02)//节点2：安防监测
        {
            Ble_Node_2.HC_SR501_status = data[0];
            Ble_Node_2.ADC_MQ2 = ((data[2] << 8) | data[1]);
            Ble_Node_2.SW420_status = data[3];
            PRINTF("Node2 => PIR:%d MQ2:%d SW420:%d\r\n",
                   Ble_Node_2.HC_SR501_status,
                   Ble_Node_2.ADC_MQ2,
                   Ble_Node_2.SW420_status);
        }
        else if(sender->v8[5] == 0x03)//节点3：辅助监测
        {
            Ble_Node_3.HC_SR501_status = 0;  // 节点3无人体传感器
            Ble_Node_3.temperature = ((data[2] << 8) | data[1]) / 10.0f;
            Ble_Node_3.humidity = ((data[4] << 8) | data[3]) / 10.0f;
            Ble_Node_3.Lux = (data[6] << 8) | data[5];
            Ble_Node_3.SW420_status = data[7];
            PRINTF("Node3 => Temp:%d Humi:%d Lux:%d SW420:%d\r\n",
                   (uint8_t)(Ble_Node_3.temperature),
                   (uint8_t)Ble_Node_3.humidity,
                   Ble_Node_3.Lux,
                   Ble_Node_3.SW420_status);
        }

        // 收到任意节点数据后，触发Edge AI融合决策
        EdgeAI_Process();
    }
