...
#define MESH_TEST_SELF_ADDR_IDX    3  //当前测试中节点地址，在device_addr数组中
#define MESH_TEST_ADDR_MAX_IDX     4  //当前测试中，有n+1个节点
...
static uint8_t           TX_DATA[10] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00,
        0x00, 0x00};
uint8_t User_TxData[10] = {0};//温度（两位） 湿度（2位）光照（2位）震动（1位）
...
void lwns_mesh_process_init(void)
{
    ...
    if(MESH_TEST_SELF_ADDR_IDX == 3)
    {
        tmos_start_task(lwns_mesh_test_taskID, MESH_EXAMPLE_TX_NODE_EVT,
