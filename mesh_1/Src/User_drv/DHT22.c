...
tmosEvents My_DHT22_Task(tmosTaskID task_id, tmosEvents events)
{
    if(events & MY_DHT22_TASK_EVENT)
    {
        ...
        Temperature = (int16_t)(DHT22_Data.temperature*10.0f);
        Humidity = (uint16_t)(DHT22_Data.humidity*10.0f);
        User_TxData[1] = Temperature & 0xff;
        User_TxData[2] = (Temperature >> 8) & 0xff;
        User_TxData[3] = Humidity & 0xff;
        User_TxData[4] = (Humidity >> 8) & 0xff;
        // 重新安排下一次触发：10ms
